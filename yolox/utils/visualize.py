...
def vis_notag(img, boxes, scores, frame_id):
    ...
    for i in range(len(boxes)):
        ...
        cv2.rectangle(img, (x0, y0), (x1, y1), color, 8)
...
def plot_tracking(image, tlwhs, obj_ids, scores=None, frame_id=0, fps=0., follow_id=0, distance=0, ids2=None, theta=0):
    ...
    cv2.putText(im, 'dis: %d theta: %.2f' % (distance, theta),
    ...
    for i, tlwh in enumerate(tlwhs):
        ...
        # color = get_color(abs(obj_id))  # 多目标跟踪

        color = (255, 0, 0) if obj_id == follow_id else (192, 192, 192)  # MOT + Gesture
