    ...
    for path in paths:
        x, y, yaw, directions = \
            generate_local_course(path.L, path.lengths,
                                  path.ctypes, maxc, step_size * maxc)  # 路径长度，路径段长度，路径类型，最大曲率，点精度
...
def set_path(paths, lengths, ctypes):  # 一小路段的曲线
    path = PATH([], [], 0.0, [], [], [], [])
    path.ctypes = ctypes
...
def SLS(x, y, phi):  # 直右直
    ...
    if y > 0.0 and 0.0 < phi < PI * 0.99:
        xd = -y / math.tan(phi) + x
        t = xd - math.tan(phi / 2.0)  # S
        u = phi  # L
        v = math.sqrt((x - xd) ** 2 + y ** 2) - math.tan(phi / 2.0)  # S
        return True, t, u, v
...
def generate_local_course(L, lengths, mode, maxc, step_size):  # 长度，路径段长度列表，路段类型，路径曲率上限，路径点之间的距离
...
def interpolate(ind, l, m, maxc, ox, oy, oyaw, px, py, pyaw, directions):  # 根据路径生成坐标点
    if m == "S":  # x, y生成
...
def generate_path(q0, q1, maxc):  # q0: 起点   q1: 终点
    dx = q1[0] - q0[0]
    dy = q1[1] - q0[1]
    dth = q1[2] - q0[2]
    c = math.cos(q0[2])
    s = math.sin(q0[2])
    x = (c * dx + s * dy) * maxc  # 旋转变换，q1点相对于q0的坐标值
    y = (-s * dx + c * dy) * maxc
