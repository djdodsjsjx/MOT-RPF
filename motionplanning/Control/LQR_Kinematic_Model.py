...
class LonController:  # 纵向控制器，速度PID
    ...
    @staticmethod
    def ComputeControlCommand(target_speed, vehicle_state, dist):  # 目标速度，车辆速度，到目标的距离
        """
...
def main():
    ...
    # x坐标，y坐标，路径角度，方向，曲率
    x_ref, y_ref, yaw_ref, direct, curv, x_all, y_all = generate_path(states)  # 路径生成
    ...
    for x, y, yaw, gear, k in zip(x_ref, y_ref, yaw_ref, direct, curv):
        ...
        while t < maxTime:
            ...
            draw.draw_omni_wheel_vehicle(x0, y0, yaw0)
            plt.axis("equal")
    ...
    # gif.save(fs, "LQR.gif", duration=50)
