...
max_steer_angle = np.deg2rad(60)  # [rad] 最大转向角度
...
class VehicleState:  # 车辆状态, 默认\beta = 0
    ...
    def UpdateVehicleState(self, delta, a, e_cg, theta_e,
                           gear=Gear.GEAR_DRIVE):  # 车辆运动学模型  转角，加速度，横向偏差，偏航角
        """
        ...
        self.yaw += self.v / chassis_d * math.tan(delta) * ts  
        self.e_cg = e_cg
...
def main2():
    ...
    while True:
        ...
        path_cnt_choose, path_cnt = 2, 0  # 两点间选择的跟踪点数设置，当前执行的跟踪点数
        ref_trajectory = None
        ...
        fs.append(f)
        if len(fs) > 200:
            gif.save(fs, "output/Robot_Follow_Ellipse_DP.gif", duration=50)
            break
